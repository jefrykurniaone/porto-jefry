...
function ExperienceCard({ exp, bullets, presentLabel, locale }: Readonly<ExperienceCardProps>) {
    const period = translatePeriod(exp.period.replace('Present', presentLabel), locale);
    return (
        <div className='relative flex gap-6 md:gap-10'>
            <div className='shrink-0 z-10 flex items-start'>
                <div className='w-8 h-8 md:w-16 md:h-16 rounded-full bg-blue-100 dark:bg-blue-950 flex items-center justify-center border-2 border-white dark:border-gray-950'>
                    <BriefcaseIcon size={14} className='text-blue-600 dark:text-blue-400 md:hidden' />
                    <BriefcaseIcon size={20} className='text-blue-600 dark:text-blue-400 hidden md:block' />
                </div>
            </div>
            <div className='flex-1 pb-2'>
                <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 mb-1'>
                    <h3 className='text-lg font-semibold text-gray-900 dark:text-white'>{exp.role}</h3>
                    <span className='text-sm text-blue-600 dark:text-blue-400 font-medium shrink-0'>{period}</span>
                </div>
                <p className='text-gray-500 dark:text-gray-400 font-medium mb-3'>{exp.company}</p>
                <ul className='space-y-1.5 mb-4'>
                    {bullets.map((b) => (
                        <li key={b} className='text-gray-600 dark:text-gray-300 text-sm flex gap-2'>
                ...
                {exp.tech && (
                    <div className='flex flex-wrap gap-2'>
                        {exp.tech.map((tech) => (
                            <span key={tech} className='text-xs px-2.5 py-1 rounded-full bg-blue-100 dark:bg-blue-950 text-blue-700 dark:text-blue-300 font-medium'>
                                {tech}
                            </span>
                        ))}
                    </div>
